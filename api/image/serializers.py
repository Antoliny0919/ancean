...
class ImageSerializer(serializers.Serializer):
  ...
  def validate(self, data):
    ...
    store_path = self.check_save_path(id, name)
    data.update({'store_path': store_path})
  ...
  def create(self, validated_data):
    ...
    """
    store_path, image_file = validated_data["store_path"], validated_data["file"]
    save_path = os.path.join(store_path, image_file.name)
    with open(save_path, mode='wb') as file:
      for chunk in image_file.chunks():
        file.write(chunk)
    source_url = save_path.replace(
      str(getattr(settings, 'BASE_DIR')),
      getattr(settings, 'SERVER_URI')
    )
